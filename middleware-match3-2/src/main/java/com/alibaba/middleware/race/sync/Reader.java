...
public final class Reader {
	...
	private static final int DATA_BUFF_SIZE = 5 * 1024 * 1024;
	private static final int DATA_QUEUE_SIZE = 80;
			...
			try {
				...
				for (int fileIndex = 1; fileIndex <= 10; ++fileIndex) {
					...
					for (;;) {
						...
						// 确保有换行符
						if (curData[len - 1] != '\n') {
							while ((curData[len++] = file.readByte()) != '\n')
								;
						}
						...
						if (file.getFilePointer() >= totalLen) {
							break;
						}
					}
			...
			case 'U':
				...
				case 'e':// scor e
					...
					if (line[pos] == '|') { // 2位
						...
						v = (v << 6) + (v << 5) + (v << 2) + ((v = line[pos++]) << 3) + (v << 1) + line[pos++]
								- twoBase;
					}
				...
				case 't': // firs t_name
					...
					items[len++] = ((long) (((line[nextPos + 16] << 5) ^ line[nextPos + 17])
							& CHINESE_SIZE)) << FIRST_NAME_OFF;
					pos = nextPos + 20; // 跳过\n
					break;
				case '_': // last _name
